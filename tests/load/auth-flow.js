...
function testSignup() {
  ...
  check(res, {
    'signup - status is 200 or 409': (r) => r.status === 200 || r.status === 409, // 409 = user exists
  ...
  if (res.status === 200) {
    ...
    check(res, {
      'profile after signup - accessible': (r) => r.status === 200 || r.status === 401,
    });
  }
}
...
function testLogin() {
  ...
  if (res.status === 200) {
    ...
    check(res, {
      'profile after login - accessible': (r) => r.status === 200 || r.status === 401,
    });
  }
}
